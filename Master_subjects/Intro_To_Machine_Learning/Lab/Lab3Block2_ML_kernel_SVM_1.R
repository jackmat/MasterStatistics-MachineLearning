...
mypred<-pamr.predict(fit = bestmodel, newx = as.matrix(test), threshold = my_threshold)
pamr.confusion(fit= model, threshold=my_threshold , extra=TRUE)
...
mycv<- cv.glmnet(x = as.matrix(t(x)), y = as.factor(y), family = "binomial", alpha = 0.5)
plot(mycv)
lambda_min<- mycv$lambda.min

bestcv_model<- glmnet(x = as.matrix(t(x)), y = as.factor(y), family = "binomial", alpha = 0.5, lambda = lambda_min)
coef(bestcv_model)
elastic_pred<-predict(bestcv_model, newx = as.matrix(t(x_t)), type ="class")
...
res<-ksvm(Conference~., data = train, kernel = "vanilladot")
my_pred<- round(predict(res, test[,-ncol(test)]), digits =0)
table( prediction = my_pred, True = y_t )
...
for(i in 1:(length(namesdata)-1)){
  formula<-  as.formula(paste(namesdata[i],"~ Conference"))
  pvalue[i]<-t.test(formula = formula, data = data, alternative="two.sided")$p.value 
  
}
ordered_pvalues<-pvalue[order(pvalue)]
plot(ordered_pvalues)

alpha <- 0.05
chosingvalues<- alpha*1:(length(namesdata)-1)/(length(namesdata)-1)
final<- ifelse(ordered_pvalues<chosingvalues, ordered_pvalues,NA)



myframe<- data.frame(names = namesdata[order(pvalue)], pvalues = ordered_pvalues, decision = final )

myframe_res<- myframe[complete.cases(myframe),1:2]
